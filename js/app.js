(async function(){
  ...
  let directiveUi={protocolId:null,stepId:null,isNew:false,isNewProtocol:false,draft:null,dirty:false,preview:false,restoreArmedUntil:0,deleteArmedUntil:0,mode:'simple',createOpen:false,manageOpen:false};
  let directiveDragStepId=null;
  ...
  const directiveProtocolDefinition=id=>{
    ...
    const savedSteps=Array.isArray(override.subtasks)&&override.subtasks.length?clone(override.subtasks):null;
    const steps=savedSteps||defaultSteps;
    const def={
  ...
  const profileBaselineProtocolConfigsForDate=dateKey=>{
    const date=new Date(`${dateKey}T12:00:00`);
    return protocolDefaults.map(base=>({
      id:base.id,name:base.name,short:base.short,icon:base.icon,start:base.start,end:base.end,xp:base.xp,required:true,enabled:true,
      systemRole:directiveSystemRoles[base.id]||'standard',activeDays:clone(directiveDefaultDays[base.id]||[0,1,2,3,4,5,6]),schedulingMode:'fixed',priority:base.id==='wake'||base.id==='shutdown'?'high':'normal',allowSkipToday:base.id!=='wake',
      subtasks:()=>base.subtasks(state.player.level).map(step=>({...clone(step),enabled:true,required:true,allowSkip:step.type==='timer',perfectRequired:true}))
    })).filter(config=>protocolEligibleForDate(config,date));
  };
  const profileExpectedProtocolRows=(dateKey,dayState)=>{
    const date=new Date(`${dateKey}T12:00:00`),configs=dateKey===currentKey()?protocolConfigsForDate(date):profileBaselineProtocolConfigsForDate(dateKey),missed=dayState==='missed',workoutExpected=configs.some(config=>config.id==='workout');
    return configs.map(config=>{
      const configuredSteps=config.subtasks(state.player.level),steps=config.id==='wake'&&workoutExpected?configuredSteps.filter(step=>step.id!=='bath'):configuredSteps,requiredSteps=steps.filter(step=>step.required!==false),stepState=missed?'missed':'pending',stepLabel=missed?'MISSED':'PENDING';
      const stepRows=steps.map(step=>`<div class="profile-directive-row ${stepState}"><span>${escapeHtml(step.title||'Directive')}</span><b>${stepLabel}</b></div>`).join('');
      return `<details class="profile-protocol-log ${missed?'failed':'pending'}"><summary><section><strong>${escapeHtml(config.name.replace(/ Protocol$/,''))}</strong><small>${escapeHtml(config.start)}–${escapeHtml(config.end)} · 0/${requiredSteps.length} required directives</small></section><b>${missed?'MISSED':'PENDING'}</b></summary><div class="profile-directive-list">${stepRows}</div></details>`;
    }).join('');
  };
  const profileDayDetailMarkup=dateKey=>{
    const {day,attendance}=profileDayRecordsFor(dateKey),xp=profileDayXp(dateKey),date=new Date(`${dateKey}T12:00:00`),result=profileDayResultLabel(dateKey),dayStats=profileDayProtocolStats(day),dayState=profileDayState(dateKey);
    const recordedRows=day?protocolConfigsForRecord(day).map(config=>{const protocol=day.protocols[config.id],boss=Boolean(protocol?.boss),name=boss?(day.weeklyBossPlan?.title?`Weekly Boss · ${day.weeklyBossPlan.title}`:'Weekly Boss'):(protocol?.name||config.name).replace(/ Protocol$/,'');const applied=Math.max(0,Number(protocol?.profileXpAppliedAmount||0)),start=protocol?.start||config.start,end=protocol?.end||config.end,progress=profileProtocolProgress(protocol);const stepRows=(protocol?.steps||[]).map(step=>`<div class="profile-directive-row ${escapeHtml(step.status||'pending')}"><span>${escapeHtml(step.title||'Directive')}</span><b>${step.status==='completed'?'DONE':step.status==='skipped'?'SKIPPED':step.status==='active'?'ACTIVE':'PENDING'}</b></div>`).join('');return `<details class="profile-protocol-log ${escapeHtml(protocol?.status||'pending')}"><summary><section><strong>${escapeHtml(name)}</strong><small>${escapeHtml(start)}–${escapeHtml(end)} · ${progress.completed}/${progress.total} required directives${applied?` · +${applied} XP`:''}</small></section><b>${profileProtocolStatusCopy(protocol||{})}</b></summary><div class="profile-directive-list">${stepRows||'<div class="profile-log-empty">The saved snapshot has no directive rows; the protocol definition remains available in Directive Studio.</div>'}</div></details>`}).join(''):'';
    const directiveRows=recordedRows||(!day&&['missed','pending'].includes(dayState)?profileExpectedProtocolRows(dateKey,dayState):'');
    ...
    const expectedCount=!day&&['missed','pending'].includes(dayState)?(dateKey===currentKey()?protocolConfigsForDate(date):profileBaselineProtocolConfigsForDate(dateKey)).filter(config=>config.required!==false).length:0;
    const protocolSummary=day?`${dayStats.cleared}/${dayStats.applicable.length} required protocols · ${dayStats.directives.completed}/${dayStats.directives.total} required directives${dayStats.excused.length?` · ${dayStats.excused.length} excused`:''}`:dayState==='missed'?`0/${expectedCount} required protocols · baseline directives shown`:dayState==='pending'?`0/${expectedCount} required protocols · directives ready`:dayState==='untracked'?'ASCEND was not active on this date.':'No protocol activity recorded.';
    const emptyProtocolCopy=dayState==='untracked'?'ASCEND was not tracking protocols on this date.':'No directive activity recorded.';
  ...
  const directiveMethodCopy=step=>{
    if(!step)return'Directive';
    if(!directiveStandardTypes.has(step.type))return'Built-in';
    if(step.type==='timer')return`Timer · ${Math.max(1,Number(step.duration||10))} min`;
    return step.type==='tap'?'Tap':'Hold';
  };
  const applyDirectiveMode=()=>{
    const advanced=directiveUi.mode==='advanced',overlay=$('#scheduleOverlay');
    if(overlay)overlay.classList.toggle('directive-advanced-mode',advanced);
    ['directiveStudioModeSimple','directiveEditorModeSimple','directiveStepModeSimple'].forEach(id=>{const button=$('#'+id);if(button){button.classList.toggle('active',!advanced);button.setAttribute('aria-pressed',String(!advanced))}});
    ['directiveStudioModeAdvanced','directiveEditorModeAdvanced','directiveStepModeAdvanced'].forEach(id=>{const button=$('#'+id);if(button){button.classList.toggle('active',advanced);button.setAttribute('aria-pressed',String(advanced))}});
    const advancedFields=$('#directiveAdvancedFields');if(advancedFields)advancedFields.hidden=!advanced;
    const advancedActions=$('#directiveAdvancedActions');if(advancedActions)advancedActions.hidden=!advanced;
    const stepAdvanced=$('#directiveStepAdvanced');if(stepAdvanced)stepAdvanced.hidden=!advanced;
    const advancedToggle=$('#directiveAdvancedToggle');if(advancedToggle)advancedToggle.hidden=advanced;
  };
  const setDirectiveMode=mode=>{directiveUi.mode=mode==='advanced'?'advanced':'simple';if(!$('#directiveStudioView')?.hidden)renderDirectiveStudio();else applyDirectiveMode();};
  const renderDirectiveStudio=()=>{
    ...
    $('#directiveProtocolList').innerHTML=definitions.length?definitions.map(def=>{
      const enabledCount=def.subtasks.filter(step=>step.enabled!==false).length,rule=def.required===false?'Optional':'Required';
      return `<article class="directive-protocol-card ${def.enabled===false?'disabled':''} ${def.custom?'custom':''}">
        <button class="directive-protocol-main" type="button" data-directive-protocol="${escapeHtml(def.id)}">
          <span class="directive-protocol-icon">${glyphMarkup(def.icon,'directive-card-glyph')}</span>
          <section><strong>${escapeHtml(def.name)}</strong><small>${escapeHtml(formatTime(def.start))}–${escapeHtml(formatTime(def.end))} · ${escapeHtml(directiveDaysCopy(def.activeDays))}</small><em>${enabledCount} directive${enabledCount===1?'':'s'}${def.enabled===false?' · Disabled':''}</em></section>
          <b class="${def.required===false?'optional':'required'}">${rule}</b>
        </button>
        <button class="directive-protocol-more" type="button" data-directive-menu="${escapeHtml(def.id)}" aria-label="More options for ${escapeHtml(def.name)}">•••</button>
        <div class="directive-protocol-menu" data-directive-menu-panel="${escapeHtml(def.id)}" hidden>
          <button type="button" data-protocol-action="edit" data-protocol-id="${escapeHtml(def.id)}">Edit</button>
          <button type="button" data-protocol-action="preview" data-protocol-id="${escapeHtml(def.id)}">Preview</button>
          <button type="button" data-protocol-action="duplicate" data-protocol-id="${escapeHtml(def.id)}">Duplicate</button>
          ${def.id==='wake'?'':`<button type="button" data-protocol-action="toggle" data-protocol-id="${escapeHtml(def.id)}">${def.enabled===false?'Enable':'Disable'}</button>`}
        </div>
      </article>`;
    }).join(''):'<div class="schedule-empty"><strong>No protocols</strong><span>Create a protocol to get started.</span></div>';
    const historyCount=state.directiveConfig?.history?.length||0,requiredCount=definitions.filter(def=>def.enabled!==false&&def.required!==false).length,optionalCount=definitions.filter(def=>def.enabled!==false&&def.required===false).length;
    $('#directiveStudioMeta').textContent=directiveUi.mode==='advanced'?`${definitions.filter(def=>def.enabled!==false).length} enabled · ${definitions.length} total · ${historyCount} saved version${historyCount===1?'':'s'}`:`${requiredCount} required · ${optionalCount} optional · ${definitions.length} total`;
    $('#directiveUndo').disabled=!historyCount;$('#directiveRestoreAll').textContent='Restore All Defaults';directiveUi.restoreArmedUntil=0;directiveUi.deleteArmedUntil=0;
    $('#directiveTemplatePanel').hidden=!directiveUi.createOpen;$('#directiveManagePanel').hidden=!directiveUi.manageOpen;$('#directiveManageToggle').textContent=directiveUi.manageOpen?'Close Data Tools':'Manage Data';$('#directiveAddProtocol').querySelector('b').textContent=directiveUi.createOpen?'Close Templates':'+ Add Protocol';
    applyDirectiveMode();
  };
  const renderDirectivePreview=()=>{const draft=directiveUi.draft,panel=$('#directivePreviewPanel');if(!draft||!directiveUi.preview){panel.hidden=true;return}panel.hidden=false;const steps=draft.subtasks.filter(step=>step.enabled!==false);$('#directivePreviewContent').innerHTML=`<div class="directive-preview-protocol"><header><strong>${escapeHtml(draft.name)}</strong><span>${escapeHtml(formatTime(draft.start))} – ${escapeHtml(formatTime(draft.end))}</span></header><p>${escapeHtml(draft.prep||'No preparation message.')}</p><div class="directive-preview-steps">${steps.map((step,index)=>`<div><b>${String(index+1).padStart(2,'0')}</b><strong>${escapeHtml(step.title)}</strong><small>${escapeHtml(directiveMethodCopy(step))}${step.required===false?' · Optional':''}${step.allowSkip?' · Skip allowed':''}</small></div>`).join('')}</div></div>`};
  const renderDirectiveEditor=()=>{
    ...
    $('#directiveEditorEyebrow').textContent=draft.custom?'CUSTOM PROTOCOL':'EDIT PROTOCOL';$('#directiveEditorTitle').textContent=draft.name;
    ...
    $('#directiveSystemRole').textContent=(draft.systemRole||'standard').toUpperCase();$('#directiveProtocolRuleSummary').textContent=`${draft.required===false?'Optional':'Required'} · ${draft.schedulingMode==='flexible'?'Flexible':'Fixed'} timing · ${String(draft.priority||'normal').toUpperCase()} priority`;$('#directiveCoreNote').hidden=!core;
    ...
    $('#directiveStepList').innerHTML=draft.subtasks.map((step,index)=>{const linked=directiveLinkedLabel(step,draft.id),method=directiveMethodCopy(step);return `<div class="directive-step-row ${step.enabled===false?'disabled':''} ${linked?'linked':''} ${step.required===false?'optional':''} ${step.allowSkip?'skippable':''}" data-directive-step="${escapeHtml(step.id)}" draggable="${linked?'false':'true'}">
      <span class="directive-step-handle" aria-hidden="true">⋮⋮</span>
      <button class="directive-step-summary" type="button" data-step-action="edit"><section><strong>${escapeHtml(step.title)}</strong><small>${escapeHtml(method)}${linked?' · Built-in':''}${step.required===false?' · Optional':' · Required'}${step.allowSkip?' · Skip allowed':''}${step.enabled===false?' · Disabled':''}</small></section><b>Edit</b></button>
      <div class="directive-step-actions directive-advanced-only"><button type="button" data-step-action="up" ${linked||index===0?'disabled':''} aria-label="Move directive up">↑</button><button type="button" data-step-action="down" ${linked||index===draft.subtasks.length-1?'disabled':''} aria-label="Move directive down">↓</button><button type="button" data-step-action="toggle" ${linked?'disabled':''}>${step.enabled===false?'On':'Off'}</button><button type="button" data-step-action="delete" ${linked?'disabled':''}>×</button></div>
    </div>`}).join('');
  ...
  const toggleDirectiveProtocolEnabledFromStudio=id=>{const def=directiveWorkingDefinition(id);if(!def||id==='wake')return;const active=activeProtocolRecord();if(active?.id===id){showBreachWarning('PROTOCOL CURRENTLY ACTIVE','Finish the active protocol before disabling it.');return}pushDirectiveHistory();def.enabled=def.enabled===false;state.directiveConfig.protocols[id]=clone({...def,custom:undefined});state.directiveConfig.updatedAt=new Date().toISOString();refreshDirectiveBlueprints();syncCurrentDayDirectiveConfig(id);state.logs.push({id:S.uid('log'),at:new Date().toISOString(),type:'system',message:`${def.name} ${def.enabled?'enabled':'disabled'} from Directive Studio.`});save();renderDirectiveStudio();showSystemNotice('save',def.enabled?'PROTOCOL ENABLED':'PROTOCOL DISABLED',`${def.name} ${def.enabled?'is active again.':'will stay out of future sessions until re-enabled.'}`,1900)};
  ...
  const wireEvents=()=>{
    ...
    $('#directiveAddProtocol').addEventListener('click',()=>{directiveUi.createOpen=!directiveUi.createOpen;if(directiveUi.createOpen)directiveUi.manageOpen=false;renderDirectiveStudio()});
    $('#directiveTemplateCancel').addEventListener('click',()=>{directiveUi.createOpen=false;renderDirectiveStudio()});
    $('#directiveTemplateGrid').addEventListener('click',event=>{const button=event.target.closest('[data-directive-template]');if(!button)return;$('#directiveTemplateSelect').value=button.dataset.directiveTemplate||'blank';openNewDirectiveProtocol(button.dataset.directiveTemplate)});
    $('#directiveManageToggle').addEventListener('click',()=>{directiveUi.manageOpen=!directiveUi.manageOpen;if(directiveUi.manageOpen)directiveUi.createOpen=false;renderDirectiveStudio()});
    ['directiveStudioModeSimple','directiveEditorModeSimple','directiveStepModeSimple'].forEach(id=>$('#'+id).addEventListener('click',()=>setDirectiveMode('simple')));
    ['directiveStudioModeAdvanced','directiveEditorModeAdvanced','directiveStepModeAdvanced'].forEach(id=>$('#'+id).addEventListener('click',()=>setDirectiveMode('advanced')));
    $('#directiveAdvancedToggle').addEventListener('click',()=>setDirectiveMode('advanced'));
    $('#directiveProtocolList').addEventListener('click',event=>{
      const menuButton=event.target.closest('[data-directive-menu]');
      if(menuButton){const id=menuButton.dataset.directiveMenu;document.querySelectorAll('[data-directive-menu-panel]').forEach(panel=>{panel.hidden=panel.dataset.directiveMenuPanel!==id||!panel.hidden});return}
      const actionButton=event.target.closest('[data-protocol-action]');
      if(actionButton){const id=actionButton.dataset.protocolId,action=actionButton.dataset.protocolAction;document.querySelectorAll('[data-directive-menu-panel]').forEach(panel=>panel.hidden=true);if(action==='edit'){openDirectiveEditor(id);return}if(action==='preview'){openDirectiveEditor(id);directiveUi.preview=true;renderDirectiveEditor();return}if(action==='duplicate'){openDirectiveEditor(id);duplicateDirectiveProtocol();return}if(action==='toggle'){toggleDirectiveProtocolEnabledFromStudio(id);return}}
      const button=event.target.closest('[data-directive-protocol]');if(button)openDirectiveEditor(button.dataset.directiveProtocol)
    });
    ...
    $('#directiveStepList').addEventListener('dragstart',event=>{const row=event.target.closest('[data-directive-step]');if(!row||row.getAttribute('draggable')!=='true')return;directiveDragStepId=row.dataset.directiveStep;row.classList.add('dragging');if(event.dataTransfer){event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',directiveDragStepId)}});
    $('#directiveStepList').addEventListener('dragover',event=>{if(!directiveDragStepId)return;const row=event.target.closest('[data-directive-step]');if(!row||row.dataset.directiveStep===directiveDragStepId||row.getAttribute('draggable')!=='true')return;event.preventDefault();if(event.dataTransfer)event.dataTransfer.dropEffect='move'});
    $('#directiveStepList').addEventListener('drop',event=>{if(!directiveDragStepId||!directiveUi.draft)return;const row=event.target.closest('[data-directive-step]');if(!row||row.dataset.directiveStep===directiveDragStepId||row.getAttribute('draggable')!=='true')return;event.preventDefault();captureDirectiveGeneralFields();const from=directiveUi.draft.subtasks.findIndex(step=>step.id===directiveDragStepId),to=directiveUi.draft.subtasks.findIndex(step=>step.id===row.dataset.directiveStep);if(from<0||to<0)return;const [moved]=directiveUi.draft.subtasks.splice(from,1);directiveUi.draft.subtasks.splice(to,0,moved);directiveUi.dirty=true;directiveDragStepId=null;renderDirectiveEditor()});
    $('#directiveStepList').addEventListener('dragend',event=>{event.target.closest('[data-directive-step]')?.classList.remove('dragging');directiveDragStepId=null});
