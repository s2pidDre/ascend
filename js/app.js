(function(){
  ...
  const renderDeveloperRun=()=>{
    ...
    const overlay=$('#developerRunOverlay');overlay.dataset.result=result.className;overlay.dataset.scenario=developerRunSession.scenario;$('#developerRunCard').classList.toggle('developer-run-card-lab',isLab);$('#developerRunActive').hidden=false;$('#developerRunResult').hidden=true;
    $('#developerRunProtocol').textContent=scenario.protocol;$('#developerRunWindow').textContent=scenario.window;$('#developerRunStatus').textContent=isLab?'LAB READY':result.status;$('#developerRunCountdown').textContent=isLab?'SESSION ACTIVE':result.countdown;$('#developerRunType').textContent=scenario.type||'CURRENT DIRECTIVE';$('#developerRunTitle').textContent=scenario.title;$('#developerRunCopy').textContent=scenario.copy;$('#developerRunDetail').innerHTML=`<span>${escapeHtml(result.summary)}</span><strong>${escapeHtml(scenario.detail)}</strong><small>${escapeHtml(developerSandboxSummary(developerRunSession.sandbox))}</small>`;$('#developerRunDetail').hidden=isLab;$('#developerRunActionLabel').textContent=scenario.action||'Hold to Resolve Test';$('#developerRunNote').textContent=scenario.note||'Test data only. Live records remain protected.';$('#developerRunFill').style.width='0%';setGlyph('developerRunGlyph',scenario.icon);
